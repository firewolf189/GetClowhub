...
require(sessionNavigationState.contains("@Published var projectBindingsByAgent"), "session navigation state should publish project bindings grouped by agent")
require(sessionNavigationState.contains("@Published var projectSessionsByAgent"), "session navigation state should publish project sessions grouped under projects")
require(sessionNavigationState.contains("@Published var generalSessionsByAgent"), "session navigation state should publish project-less sessions separately")
require(viewModel.contains("get { sessionState.projectBindingsByAgent }"), "view model should keep a compatibility facade for project bindings")
require(viewModel.contains("get { sessionState.projectSessionsByAgent }"), "view model should keep a compatibility facade for project sessions")
require(viewModel.contains("get { sessionState.generalSessionsByAgent }"), "view model should keep a compatibility facade for general sessions")
require(sessionPersistence.contains("func openProject(forAgent agentId: String)"), "view model should expose an Open Project action through its session persistence extension")
...
require(chatHelpers.contains("ProjectSessionContextBuilder.message"), "sendChatMessage should use the extracted project context builder")
