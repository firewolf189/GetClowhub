...
assertContains(dashboard, "workspaceSidebarExpanded || hasWorkspaceDetailPanel || isWorkspaceSidebarOpening || isWorkspaceSidebarClosing", "Outputs sidebar content must stay mounted while opening or closing")
...
assertContains(rightInspectorSplit, "let shouldDeferSidebarRootUpdate = isCollapsingSidebar || (isAnimatingSidebar && !currentIsSidebarExpanded)", "Outputs sidebar root updates must defer while collapsing")
assertContains(rightInspectorSplit, "if !shouldDeferSidebarRootUpdate {", "Outputs sidebar content must stay mounted while closing")
assertContains(rightInspectorSplit, "completion: isCollapsingSidebar ? {\n                self.sidebarHost.rootView = AnyView(sidebar.environment(\\.rightInspectorSidebarWidthCoordinator, self.sidebarWidthCoordinator))\n                self.onSidebarCollapseFinished?()", "Outputs sidebar root and SwiftUI state must update only after close animation")
...
assertContains(workspaceInspectorPane, "private func shouldShowOutputItem", "right sidebar must filter the workspace to output artifacts")
assertContains(workspaceInspectorPane, "\"USER.md\", \"BOOTSTRAP.md\", \"HEARTBEAT.md\", \"TOOLS.md\"", "Outputs filtering must exclude user/context documents")

assertContains(config, "struct GatewaySettingsGroup", "Gateway settings group must exist")
assertContains(config, "Text(localizedString(\"Gateway\"))", "Gateway heading must be shown")
