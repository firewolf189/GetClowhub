...
assertContains(dashboard, "private final class DashboardWorkspaceSplitController: NSViewController", "Outputs sidebar shell must be owned by AppKit")
assertContains(dashboard, "private let sidebarAnimationDuration: TimeInterval = 0.30", "right split and titlebar accessory must share one smoother animation duration")
assertContains(dashboard, "sidebarWidthConstraint?.animator().constant", "Outputs sidebar must animate one AppKit width constraint")
...
assertContains(dashboard, "guard hasInstalledLayout, hasAppliedInitialLayout, !isAnimatingSidebar else { return }", "layout passes during Outputs animation must not snap the middle pane to the final width")
assertContains(dashboard, "private var sidebarAnimationGeneration = 0", "Outputs sidebar animation should invalidate stale completions")
assertContains(dashboard, "private var onSidebarCollapseFinished: (() -> Void)?", "Outputs sidebar collapse completion must be owned by the AppKit split controller")
assertContains(dashboard, "private var onSidebarExpandFinished: (() -> Void)?", "Outputs sidebar expand completion must be owned by the AppKit split controller")
assertContains(dashboard, "private var lastExpandRequestID = 0", "Outputs sidebar controller must track direct expand request ids")
assertContains(dashboard, "private var lastCollapseRequestID = 0", "Outputs sidebar controller must track direct collapse request ids")
assertContains(dashboard, "let shouldCollapseFromRequest = collapseRequestID != lastCollapseRequestID", "Outputs sidebar must start closing from a direct AppKit request")
assertContains(dashboard, "let shouldExpandFromRequest = expandRequestID != lastExpandRequestID", "Outputs sidebar must start opening from a direct AppKit request")
assertContains(dashboard, "let isCollapsingSidebar = (shouldAnimate && currentIsSidebarExpanded && !isSidebarExpanded && hasAppliedInitialLayout) || (shouldCollapseFromRequest && hasAppliedInitialLayout)", "Outputs sidebar must identify closing before replacing the SwiftUI root")
assertContains(dashboard, "let shouldDeferSidebarRootUpdate = isCollapsingSidebar || (isAnimatingSidebar && !currentIsSidebarExpanded)", "Outputs sidebar content must stay mounted during follow-up updates while closing")
assertContains(dashboard, "if !shouldDeferSidebarRootUpdate {\n            sidebarHost.rootView = sidebar\n        }", "Outputs sidebar content must stay mounted while closing")
assertContains(dashboard, "self.sidebarHost.rootView = sidebar\n                self.onSidebarCollapseFinished?()", "Outputs sidebar root and SwiftUI state must update only after close animation")
assertContains(dashboard, "if isAnimatingSidebar && !currentIsSidebarExpanded && isSidebarExpanded {\n            return\n        }", "Outputs sidebar must not reverse a requested close while SwiftUI still reports expanded business state")
...
assertContains(dashboard, "guard self.sidebarAnimationGeneration == animationID else { return }", "stale Outputs sidebar animation completions must not hide the current sidebar")
assertContains(dashboard, "animateSidebarWidth(to: targetWidth)", "Outputs sidebar width changes should animate through the AppKit width constraint")
assertContains(dashboard, "private let sidebarRail = NSView()", "Outputs sidebar must use an AppKit inspector rail")
assertContains(dashboard, "private let sidebarSeparator = NSBox()", "Outputs sidebar rail must own a native separator")
assertContains(dashboard, "private let sidebarClipView = NSView()", "Outputs sidebar rail must own a dedicated clipping view")
assertContains(dashboard, "sidebarRail.clipsToBounds = true", "Outputs sidebar rail must clip while its width animates")
assertContains(dashboard, "sidebarClipView.clipsToBounds = true", "Outputs sidebar clip view must prevent content flashing outside the rail")
assertContains(dashboard, "sidebarRail.addSubview(sidebarSeparator)", "Outputs separator must be installed inside the rail")
assertContains(dashboard, "sidebarRail.addSubview(sidebarClipView)", "Outputs clip view must be installed inside the rail")
assertContains(dashboard, "sidebarClipView.addSubview(sidebarHost.view)", "Outputs SwiftUI host must live inside the clip view")
assertContains(dashboard, "sidebarRail.widthAnchor.constraint(equalToConstant: 0)", "Outputs sidebar width animation must target the whole inspector rail")
assertContains(dashboard, "contentHost.view.trailingAnchor.constraint(equalTo: sidebarRail.leadingAnchor)", "Outputs inspector rail and middle pane must share one moving Auto Layout boundary")
assertContains(dashboard, "sidebarRail.trailingAnchor.constraint(equalTo: view.trailingAnchor)", "Outputs inspector rail must stay pinned to the window edge")
assertContains(dashboard, "sidebarSeparator.leadingAnchor.constraint(equalTo: sidebarRail.leadingAnchor)", "Outputs separator must sit on the moving boundary")
assertContains(dashboard, "sidebarClipView.leadingAnchor.constraint(equalTo: sidebarSeparator.trailingAnchor)", "Outputs content must start after the separator")
assertContains(dashboard, "sidebarHost.view.leadingAnchor.constraint(equalTo: sidebarClipView.leadingAnchor)", "Outputs SwiftUI host must be anchored inside the clip view")
assertContains(dashboard, "sidebarContentWidthConstraint?.constant", "Outputs SwiftUI host width must be pre-sized separately from the animated rail")
assertContains(dashboard, "separatorWidthConstraint.priority = .fittingSizeCompression", "Outputs separator width must yield while the rail collapses to zero width")
assertNotContains(dashboard, "sidebarRail.isHidden", "Outputs inspector rail must stay mounted at zero width so the width animation remains smooth")
assertNotContains(dashboard, "sidebarHost.view.isHidden", "Outputs sidebar must not toggle the SwiftUI host visibility during titlebar button animation")
assertNotContains(dashboard, "sidebarContainer", "Outputs sidebar must use the fuller rail/clip/separator structure instead of the simpler B container")
assertNotContains(dashboard, "NSSplitViewItem", "Outputs sidebar must not rely on resident split item state")
assertNotContains(dashboard, "splitView.animator().setPosition", "Outputs sidebar must not animate a split divider")
assertNotContains(dashboard, "canCollapse", "Outputs sidebar must not rely on split-item collapse behavior")
assertNotContains(dashboard, "prepareSidebarForExpansion", "Outputs sidebar must not relayout in a separate pre-expansion phase")
assertNotContains(dashboard, ".isCollapsed", "Outputs sidebar must use zero divider width instead of collapsed state")
