...
let workStatusURL = root.appendingPathComponent("OpenClawInstaller/Features/Chat/Views/WorkStatusHeader.swift")
let shimmeringStatusURL = root.appendingPathComponent("OpenClawInstaller/DesignSystem/Components/ShimmeringStatusText.swift")
...
let chatHelpers = try String(contentsOf: chatHelpersURL, encoding: .utf8)
let chatMessage = try String(contentsOf: chatMessageURL, encoding: .utf8)
...
require(chatMessage.contains("let completedAt: Date?"), "chat messages should persist a terminal work end time")
require(chatHelpers.contains("completedAt: resolvedCompletedAt"), "message updates should stamp the terminal work end time")
