...
// This guard exercises the real PluginCatalogService app logic. `swift` can
// only interpret a single file, so we compile the app sources together with an
...
let workDir = fm.temporaryDirectory
    .appendingPathComponent("verify_plugin_catalog_service-\(UUID().uuidString)")
