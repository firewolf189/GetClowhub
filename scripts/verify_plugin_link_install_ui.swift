...
expect(!pluginsTab.contains("await viewModel.installPlugin(spec: spec, link: isLink)"), "Link install should pass the typed spec without --link")
...
expect(viewModel.contains("openclaw plugins install"), "manual install should use OpenClaw plugin install")
