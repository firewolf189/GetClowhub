...
class DashboardViewModel: ObservableObject {
    ...
    let sessionNavigationViewModel = SessionNavigationViewModel()
    let channelListViewModel = ChannelListViewModel()
    let weixinChannelViewModel = WeixinChannelViewModel()
    let modelSettingsViewModel = ModelSettingsViewModel()
    let cronJobsViewModel = CronJobsViewModel()
    let statusSummaryViewModel = StatusSummaryViewModel()
    let pluginListViewModel: PluginListViewModel
    ...
    var chatRunLifecycleCoordinator: ChatRunLifecycleCoordinator { chatViewModel.chatRunLifecycleCoordinator }
    ...
    var attachmentProcessor: AttachmentProcessor { chatViewModel.attachmentProcessor }
    ...
    var hasUnsavedChanges: Bool {
        ...
        return editedPort != String(s.gatewayPort)
            ...
            || configuredCustomProviders != s.customProviders
    }
    ...
    init(
        ...
        self.pluginListViewModel = PluginListViewModel(
            openclawService: openclawService,
            notifySuccess: { _ in },
            notifyError: { _ in }
        )
        ...
        pluginListViewModel.configureNotifications(
            notifySuccess: { [weak self] message in self?.showSuccessMessage(message) },
            notifyError: { [weak self] message in self?.showErrorMessage(message) }
        )
        skillsViewModel.configureNotifications(
            notifySuccess: { [weak self] message in self?.showSuccessMessage(message) },
            notifyError: { [weak self] message in self?.showErrorMessage(message) }
        )
        ...
        channelListViewModel.objectWillChange
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.objectWillChange.send() }
            .store(in: &cancellables)

        weixinChannelViewModel.objectWillChange
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.objectWillChange.send() }
            .store(in: &cancellables)

        modelSettingsViewModel.objectWillChange
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.objectWillChange.send() }
            .store(in: &cancellables)

        cronJobsViewModel.objectWillChange
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.objectWillChange.send() }
            .store(in: &cancellables)

        statusSummaryViewModel.objectWillChange
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.objectWillChange.send() }
            .store(in: &cancellables)

        pluginListViewModel.objectWillChange
            .receive(on: RunLoop.main)
            .sink { [weak self] _ in self?.objectWillChange.send() }
            .store(in: &cancellables)
        ...
        gatewayClient.$connectionState
            ...
            .sink { [weak self] state in
                self?.chatViewModel.handleGatewayConnectionState(state)
            }
            .store(in: &cancellables)
        ...
        openclawService.$status
            ...
            .sink { [weak self] status in
                ...
                if status == .running {
                    if self.gatewayClient.connectionState.needsConnect {
    ...
    func moveTaskToBackground(_ msgId: UUID) {
        chatViewModel.moveTaskToBackground(msgId)
    }

    func finishCancelledChatRun(_ messageId: UUID) {
        chatViewModel.finishCancelledChatRun(messageId)
    }
    ...
    func appendBackgroundNotification(agentId: String, agentEmoji: String?, completed: Bool, msgId: UUID) {
        chatViewModel.appendBackgroundNotification(
            agentId: agentId,
            agentEmoji: agentEmoji,
            completed: completed,
            msgId: msgId
    ...
    func recoverInFlightRunsOnLaunch() {
        chatViewModel.recoverInFlightRunsOnLaunch()
    }

    func scheduleChatRunReconciliation(messageId: UUID) {
        chatViewModel.scheduleChatRunReconciliation(messageId: messageId)
    }

    func registerInFlightRun(_ run: ChatRunState, agentEmoji: String?) {
        chatViewModel.registerInFlightRun(run, agentEmoji: agentEmoji)
    }
    ...
    var autoBackgroundAfterSeconds: Int? { chatViewModel.autoBackgroundAfterSeconds }
    ...
    var plugins: [PluginInfo] {
        get { pluginListViewModel.plugins }
        set { pluginListViewModel.plugins = newValue }
    }
    var isLoadingPlugins: Bool {
        get { pluginListViewModel.isLoadingPlugins }
        set { pluginListViewModel.isLoadingPlugins = newValue }
    }
    var pluginCatalog: [PluginCatalogItem] {
        get { pluginListViewModel.pluginCatalog }
        set { pluginListViewModel.pluginCatalog = newValue }
    }
    var isLoadingPluginCatalog: Bool {
        get { pluginListViewModel.isLoadingPluginCatalog }
        set { pluginListViewModel.isLoadingPluginCatalog = newValue }
    }
    var installingCatalogPluginName: String? {
        get { pluginListViewModel.installingCatalogPluginName }
        set { pluginListViewModel.installingCatalogPluginName = newValue }
    }
    var pluginCatalogError: String? {
        get { pluginListViewModel.pluginCatalogError }
        set { pluginListViewModel.pluginCatalogError = newValue }
    }

    var hasLoadedPluginCatalog: Bool {
        get { pluginListViewModel.hasLoadedPluginCatalog }
        set { pluginListViewModel.hasLoadedPluginCatalog = newValue }
    }

    // Channels
    var channels: [ChannelInfo] {
        get { channelListViewModel.channels }
        set { channelListViewModel.channels = newValue }
    }
    var isLoadingChannels: Bool {
    ...
    var weixinQRImage: NSImage? {
        get { weixinChannelViewModel.qrImage }
        set { weixinChannelViewModel.qrImage = newValue }
    }
    var weixinLoginStatus: WeixinLoginStatus {
        get { weixinChannelViewModel.loginStatus }
        set { weixinChannelViewModel.loginStatus = newValue }
    }
    var weixinLoginProcess: Process? {
        get { weixinChannelViewModel.loginProcess }
        set { weixinChannelViewModel.loginProcess = newValue }
    }

    // Models
    var models: [ModelInfo] {
        get { modelSettingsViewModel.models }
        set { modelSettingsViewModel.models = newValue }
    }
    var modelOverview: ModelOverview {
    ...
    var maxConcurrentTasks: Int {
        get { modelSettingsViewModel.maxConcurrentTasks }
        set { modelSettingsViewModel.maxConcurrentTasks = newValue }
    }
    ...
    var fallbackModels: [String] {
        get { modelSettingsViewModel.fallbackModels }
        set { modelSettingsViewModel.fallbackModels = newValue }
    }
    var imageFallbackModels: [String] {
        get { modelSettingsViewModel.imageFallbackModels }
        set { modelSettingsViewModel.imageFallbackModels = newValue }
    }
    var isLoadingModels: Bool {
        get { modelSettingsViewModel.isLoadingModels }
        set { modelSettingsViewModel.isLoadingModels = newValue }
    }
    var availableModelGroups: [ProviderModelGroup] {
        get { modelSettingsViewModel.availableModelGroups }
        set { modelSettingsViewModel.availableModelGroups = newValue }
    }
    var availableModelsForSettings: [ModelOption] {
        get { modelSettingsViewModel.availableModelsForSettings }
        set { modelSettingsViewModel.availableModelsForSettings = newValue }
    }

    // Cron Jobs
    var cronJobs: [CronJobInfo] {
        get { cronJobsViewModel.cronJobs }
        set { cronJobsViewModel.cronJobs = newValue }
    }
    var isLoadingCronJobs: Bool {
        get { cronJobsViewModel.isLoadingCronJobs }
        set { cronJobsViewModel.isLoadingCronJobs = newValue }
    }
    var hasLoadedCronJobs: Bool {
        get { cronJobsViewModel.hasLoadedCronJobs }
        set { cronJobsViewModel.hasLoadedCronJobs = newValue }
    }
    var cronJobsLoadError: String? {
        get { cronJobsViewModel.cronJobsLoadError }
        set { cronJobsViewModel.cronJobsLoadError = newValue }
    }

    // Sessions Summary (for Status tab monitoring)
    var sessionsSummary: SessionsSummary? {
        get { statusSummaryViewModel.sessionsSummary }
        set { statusSummaryViewModel.sessionsSummary = newValue }
    }
    var isLoadingSessionsSummary: Bool {
        get { statusSummaryViewModel.isLoadingSessionsSummary }
        set { statusSummaryViewModel.isLoadingSessionsSummary = newValue }
    }
    ...
    var skills: [SkillInfo] {
        get { skillsViewModel.skills }
        set { skillsViewModel.skills = newValue }
    }
    var skillsSummary: SkillsSummary {
        get { skillsViewModel.skillsSummary }
        set { skillsViewModel.skillsSummary = newValue }
    }
    var isLoadingSkills: Bool {
        get { skillsViewModel.isLoadingSkills }
        set { skillsViewModel.isLoadingSkills = newValue }
    }
    var selectedSkillDetail: SkillDetailInfo? {
        get { skillsViewModel.selectedSkillDetail }
        set { skillsViewModel.selectedSkillDetail = newValue }
    }
    var isLoadingSkillDetail: Bool {
        get { skillsViewModel.isLoadingSkillDetail }
        set { skillsViewModel.isLoadingSkillDetail = newValue }
    }
    var removingSkillName: String? {
        get { skillsViewModel.removingSkillName }
        set { skillsViewModel.removingSkillName = newValue }
    }
    var skillCatalog: [SkillCatalogItem] {
        get { skillsViewModel.skillCatalog }
        set { skillsViewModel.skillCatalog = newValue }
    }
    var isLoadingSkillCatalog: Bool {
        get { skillsViewModel.isLoadingSkillCatalog }
        set { skillsViewModel.isLoadingSkillCatalog = newValue }
    }
    var installingCatalogSkillName: String? {
        get { skillsViewModel.installingCatalogSkillName }
        set { skillsViewModel.installingCatalogSkillName = newValue }
    }
    var isInstallingManualSkill: Bool {
        get { skillsViewModel.isInstallingManualSkill }
        set { skillsViewModel.isInstallingManualSkill = newValue }
    }
    var skillCatalogError: String? {
        get { skillsViewModel.skillCatalogError }
        set { skillsViewModel.skillCatalogError = newValue }
    }

    var hasLoadedSkillCatalog: Bool {
        get { skillsViewModel.hasLoadedSkillCatalog }
        set { skillsViewModel.hasLoadedSkillCatalog = newValue }
    }
    ...
    var chatSessionStore: ChatSessionStore { sessionNavigationViewModel.chatSessionStore }
    ...
    func rewindToMessage(id messageId: UUID, replacementText: String? = nil) {
        let agentId = selectedAgentId
        guard let message = (chatMessagesByAgent[agentId] ?? []).first(where: { $0.id == messageId }) else {
            self.rewindError = "找不到该消息，无法回滚"
            return
        }
        rewindToMessage(message, replacementText: replacementText)
    }
    ...
    func recomputeIsSendingMessage() {
        chatViewModel.recomputeIsSendingMessage()
    }
    ...
    func updateActiveStreamState(
        ...
        chatViewModel.updateActiveStreamState(
            msgId: msgId,
    ...
    func clearActiveStreamState(_ msgId: UUID) {
        chatViewModel.clearActiveStreamState(msgId)
    }

    func clearTaskTracking(_ msgId: UUID) {
        chatViewModel.clearTaskTracking(msgId)
    }

    func scheduleAutomaticBackground(for messageId: UUID) {
        chatViewModel.scheduleAutomaticBackground(for: messageId)
    }

    func retryChatConnection(for messageId: UUID) {
        chatViewModel.retryChatConnection(for: messageId)
    }
    ...
    func findMessage(byId msgId: UUID) -> ChatMessage? {
        chatViewModel.findMessage(byId: msgId)
    }
