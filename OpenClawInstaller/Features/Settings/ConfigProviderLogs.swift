...
extension DashboardViewModel {
    ...
    func addCustomProvider(from preset: ProviderPreset, apiKey: String) {
        ...
        selectCustomProvider(provider)
        providerModelFetchMessage = ""
    }

    func selectCustomProvider(_ provider: ConfiguredCustomProvider) {
        #if REQUIRE_LOGIN
        editedActiveServiceSource = "custom"
        #endif
        editedSelectedProviderKey = provider.key
        editedModelBaseUrl = provider.baseUrl
        editedModelApiKey = provider.apiKey
        editedProviderApi = provider.api
        editedConfiguredModels = provider.models
        providerModelFetchMessage = ""
        refreshAvailableModelsForCurrentProvider()
    }
    ...
    func deleteCustomProvider(_ provider: ConfiguredCustomProvider) {
        ...
        if editedSelectedProviderKey == provider.key {
            if let fallback = configuredCustomProviders.first {
                selectCustomProvider(fallback)
            } else {
    ...
    func syncSelectedCustomProviderIntoRegistry() {
        ...
        let displayName = configuredCustomProviders.first(where: { $0.key == providerKey })?.displayName
            ?? availableProviders.first(where: { $0.key == providerKey })?.displayName
            ?? providerKey
        let provider = ConfiguredCustomProvider(
            key: providerKey,
