...
DEFAULT_API_KEY = "NpJb3UkgD5nelMyrhvY9HmjAETwR4xaP"
...
def build_pending_url(base_url: str, page: Optional[int] = None, size: Optional[int] = None) -> str:
    url = f"{normalize_base_url(base_url)}/api/external/tax/exemption/pending"
    params = {}
    if page is not None:
...
def cmd_pending(args: argparse.Namespace) -> None:
    ...
    if args.all:
        size = args.size or DEFAULT_PENDING_ALL_SIZE
        ...
        for offset in range(args.max_pages):
            ...
            response = request_json("GET", build_pending_url(args.base_url, page, size), api_key)
            if first_response is None:
                first_response = response
            data = response.get("data") or {}
            total_count = int(data.get("count") or 0)
            enable_create = bool(data.get("enable_create"))
            ...
            if total_count and len(all_items) >= total_count:
                break
        output = first_response or {"code": 0, "message": "", "data": {}}
        output["data"] = {
            "count": total_count,
    ...
    print(json.dumps(request_json("GET", build_pending_url(args.base_url, args.page, args.size), api_key), ensure_ascii=False, indent=2))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pending.add_argument("--all", "-all", action="store_true", help="Fetch all pending pages.")
    pending.add_argument("--max-pages", type=int, default=100, help="Maximum pages to fetch with --all.")
