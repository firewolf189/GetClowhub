...
Detailed expansion of the six-step Webwright loop, adapted for GetClowHub/OpenClaw.
...
here by your native abilities (PNG inspection + reasoning against
...
- Print URL, title, and `aria_snapshot()` for the region of interest at
  every step.
- Inspect saved PNGs to confirm UI state when ARIA evidence is
  ambiguous.
