...
Playwright workspace. In GetClowHub/OpenClaw, replace that loop directly:
run one shell command at a time through the available terminal tool, observe
its output, then decide the next command. Do not wrap commands in JSON; use
normal tool calls and keep generated files inside the chosen workspace.
...
own native abilities**: inspect saved screenshots with the available image or
file-reading tools and verify success against `plan.md` yourself. No
`OPENAI_API_KEY` or other model API keys are required for the skill workflow.
...
- **CLI tool (parameterized).** `final_script.py` is a reusable CLI: one
  ...
  into a CLI", etc. See `references/cli_tool_mode.md`.

## Prerequisites (one-time)

From the active project or Webwright workspace:
...
2. **Explore.** Run scratch Playwright scripts (heredoc-style; see
   `references/playwright_patterns.md`) to discover stable selectors and
   confirm filter controls exist. Inspect saved PNGs with the available image
   or file-reading tool. Print ARIA snapshots, URLs, titles, and visible labels
...
5. **Self-verify** (this replaces `webwright.tools.self_reflection`). Walk
   ...
   - For each CP, identify a screenshot path AND/OR a log line that proves
     it. Inspect each cited PNG and confirm the evidence is unambiguous (the
...
- `references/playwright_patterns.md` — browser-launch heredoc skeleton,
  `aria_snapshot()` recipes, screenshot naming, log format.
- `references/workflow.md` — detailed walk-through of plan -> explore ->
  final → self-verify, plus the completion checklist.
- `references/cli_tool_mode.md` — contract for CLI tool mode
