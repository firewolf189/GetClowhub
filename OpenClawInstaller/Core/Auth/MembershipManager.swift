...
class MembershipManager: ObservableObject {
    ...
    @Published var hasLoadedKeysBilling: Bool = false
    ...
    func setup(authManager: AuthManager) {
        ...
        authManager.logoutSubject
            ...
            .sink { [weak self] in
                ...
                self?.apiKeys = []
                self?.keysBilling = []
                self?.hasLoadedKeysBilling = false
                self?.syncState = .idle
            }
            .store(in: &cancellables)
    }
    ...
    func fetchKeysBilling() async {
        ...
        guard !isBillingLoading else { return }
    ...
    func allowedGetClawHubModelIDs() -> [String] {
        if let activeKey = apiKeys.last(where: { $0.isActive }), !activeKey.models.isEmpty {
            return activeKey.models
        }
        if let membership, !membership.models.isEmpty {
            return membership.models
        }
        return []
    }

    func filterAllowedGetClawHubModels(_ models: [PresetModel]) -> [PresetModel] {
        let allowedModelIDs = allowedGetClawHubModelIDs()
        guard !allowedModelIDs.isEmpty else {
            return models
        }

        // Match case-insensitively to absorb backend/preset casing drift.
        let allowedLowercased = Set(allowedModelIDs.map { $0.lowercased() })
